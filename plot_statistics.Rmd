...
plotSecurityPeriodPriceProgression <- function (transactions, dividends, period_num, state_num, labels){
  ...
  dividends <- dividends[dividends$state_num == state_num,]
  abline(h = c(EPSILON, max(dividends$dividend)), lwd = 3, lty = 2, col = c("red", "forestgreen"))
}
...
DB_NAME = "market5NoBacklogRep1Mini.db"
    ...
    if (length(realized_periods) > 0){
      for (period_num in realized_periods) {
        ...
        if (period_num == realized_periods[1]){
          ...
          abline(h = c(EPSILON, dividend_payoffs, dividend_payoffs / 2), lty = 2, lwd = c(3, rep(3, trader_types), rep(1, trader_types)), col = c("red", rep("forestgreen", trader_types),rep("purple", trader_types)))
        }
        # Following periods
        else{
    ...
    if (length(unrealized_periods) > 0){
      for (period_num in unrealized_periods) {
        ...
        if (period_num == unrealized_periods[1]){
          plot(cur_transactions$transaction_num, cur_transactions$price, pch = 19, col = "orange", type = "o", xlim = c(0, max(transactions[transactions$state_num == security_num & transactions$period_num %in% unrealized_periods,]$transaction_num)), ylim = c(0, 1), xlab = "Transaction Number", ylab = "Price", main = paste("Security", security_num, "Unrealized Price By Transaction"))
          dividend_payoffs <- unique(dividends[dividends$state_num == security_num,]$dividend)
          trader_types <- length(dividend_payoffs)
          abline(h = c(EPSILON, dividend_payoffs, dividend_payoffs / 2), lty = 2, lwd = c(3, rep(3, trader_types), rep(1, trader_types)), col = c("red", rep("forestgreen", trader_types), rep("purple", trader_types)))
        # Following periods
          ...
          lines(cur_transactions$transaction_num, cur_transactions$price, pch = 19, type = "o", col = "orange")
        }
      }
    }
  }
