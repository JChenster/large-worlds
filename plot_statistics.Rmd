    ...
    if (length(realized_periods) > 0){
      for (period_num in realized_periods) {
        ...
        if (period_num == realized_periods[1]){
          ...
          trader_types <- length(dividend_payoffs)
          abline(h = c(EPSILON, dividend_payoffs, dividend_payoffs / 2), lty = 2, lwd = c(3, rep(3, trader_types), rep(2, trader_types)), col = c("red", rep("forestgreen", trader_types),rep("orange", trader_types)))
        }
        # Following periods
        else{
    ...
    if (length(unrealized_periods) > 0){
      for (period_num in unrealized_periods) {
        ...
        if (period_num == unrealized_periods[1]){
          ...
          dividend_payoffs <- unique(dividends[dividends$state_num == security_num,]$dividend)
          trader_types <- length(dividend_payoffs)
          abline(h = c(EPSILON, dividend_payoffs, dividend_payoffs / C), lty = 2, lwd = c(3, rep(3, trader_types), rep(2, trader_types)), col = c("red", rep("forestgreen", trader_types), rep("orange", trader_types)))
        # Following periods
        }
        else{
